...
function(input, output, session) {
  ...
  output$calibration_points <- renderUI({
    ui_num_od_read(manual_od_readings())
  })

  # Check if no pio reactor ODs have given, if so calculate these.
  complete_od_data <- reactive(no_pio_ods_check(
    manual_od_readings(),
    od_data_frame(),
    input$x_pio_ods
  ))

  manual_lm_models <- reactive(
    split_od_per_reactor(
      complete_od_data(), as.logical(input$fixed_intercept),
      input$origin_point
    )
  )

  # Use linear models to predict "true" values for PioReactors
  od_calibration_readings <- reactive(predict_calibrated_ods(
    manual_lm_models(),
    od_data_frame()
  ))
  ## TODO - Change the od_data_frame() to be the return from the no_pio_ods_check
  ...
  output$download_table <- downloadHandler(
    ...
    content = function(file) {
      write.table(write_calibrate_od_to_pio_format(od_calibration_readings()),
        file,
        row.names = FALSE,
        col.names = TRUE,
        sep = ",",
